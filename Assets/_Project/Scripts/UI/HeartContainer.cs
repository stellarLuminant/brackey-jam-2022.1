...
public class HeartContainer : MonoBehaviour
{
	...
	[Header("Debugging")]
	[SerializeField] bool _triggerInit;
	[SerializeField] int _triggerInitHealth = 3;
	[SerializeField] List<HeartUI> hearts;
	...
	private void BaseAddHeart(bool animation)
	{
		// Create heart object
		GameObject heartObject = Instantiate(heartPrefab, transform);
		HeartUI heart = heartObject.GetComponent<HeartUI>();
	...
	public IEnumerator RemoveHeart(int amount)
	{
		...
		for (int i = 0; i < amount; i++)
		{
			...
			HeartUI heart = hearts[hearts.Count - 1];
			hearts.Remove(heart);
			heart.Destroy();
